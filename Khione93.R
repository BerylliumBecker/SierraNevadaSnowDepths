...
adj_biased_SWE <- function(DT, Station_ID, trusted, biased,
  ...
  #   Station_ID: A 3-letter abbreviation for the station location.
...
est_missing_SWE <- function(DT, Station_ID, SWE = "Adjusted_SWE",
  ...
  #   first_mo: The first month that should be used as a predictor of
...
monthly_to_final <- function(monthly, Station_ID, repository = Sierra_SWE,
  ...
  #                  in Excel from the individual "_Differences" files and
  #                  exported.
  #   n: The number of iterations to run through the random number
...
publicationTheme <- theme(
    ...
    strip.text = element_text(size = 9L),
    axis.ticks.x = element_line(color = "black", linetype = 1),
    axis.ticks.y = element_line(color = "black", linetype = 1),
    axis.text = element_text(color = "black", size = 9L),
    axis.title = element_text(size = 9L))
...
postscript(file = "Fig S21 - Snow Density vs. SWE.eps",
           width = 3.54, height = 5.625, pointsize = 9L,
...
# plot data
FigS20 <- ggplot(data = Sierra_SWE[SWE_n >= min_months],
         aes(x = Elevation_m,
             y = Avg_SWE_m)) +
    ...
    facet_wrap(~Month)
# export figure
postscript(file = "Fig S20 - SWE by Month.eps",
           width = 6.5, height = 6.5 / 1.618, pointsize = 9L,
           horizontal = FALSE, family = "Helvetica",
           paper = "special")
multiplot(FigS20, cols = 1)
dev.off()

# Plot monthly average snow depths
# extract monthly average SWE
SWEvector <- data.table(SWE_m = Sierra_SWE[, Avg_SWE_m])
# predict avg snow pack density for these values
predictedSnowDensities <- predict.lm(msd_lm, SWEvector)
SWEvector[, SnowDensity := predictedSnowDensities]
# calculate predicted snow depths
SWEvector[, SnowDepth := SWE_m / SnowDensity]
# transfer values to Sierra_SWE
Sierra_SWE[, Avg_Snow_Depth_m := SWEvector[, SnowDepth]]
# plot average snow depth by month
FigS1 <- ggplot(data = Sierra_SWE[SWE_n >= min_months],
       aes(x = Elevation_m,
           y = Avg_Snow_Depth_m)) +
  theme(panel.border = element_rect(fill = NA, color = "black",
                                    linetype = 1),
  ...
  scale_x_continuous(name = "Elevation (m)",
                     limits = c(lower_xlim, upper_xlim)) +
  scale_y_continuous(name = "Average Snow Depth (m)",
                     limits = c(0, 3)) +
  geom_smooth(method = "lm", na.rm = TRUE, fullrange = TRUE,
              se = FALSE) +
  geom_point(shape = 22, size = 1, color = "black",
  ...
  facet_wrap(~Month)
# Export figure
postscript(file = "Fig S1 - Snow Depth.eps",
           width = 6.5, height = 6.5 / 1.618, pointsize = 9L,
           horizontal = FALSE, family = "Helvetica",
           paper = "special")
multiplot(FigS1, cols = 1)
dev.off()

# How many years long are the March and April records?
Sierra_SWE[SWE_n >= 10 & (Month == "March" | Month == "April"),
           median(SWE_n), by = "Month"]
...
# 1- injest text file of sample info in CRONUScalc format
  marrero <- fread("Marrero_simple_YESerosion_NOsnow_GchronPaper.txt") 
...
# 5- Set the coefficient to multiply the reconstructed SWEs by...
  ...
  historical_snow_coef <- c(0.0, 1.0)
...
# CALCULATE SNOW DEPTHS FOR PAPER INTRODUCTION------------------------
snow_depth
